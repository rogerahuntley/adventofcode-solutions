let [crates, instructions] = input.trimEnd().split('\n\n').map(half => half.split('\n'))

const piles = {}

// place the crates in the piles (in reverse, for stacking)
crates = crates.slice(0, -1).reverse().map(row => {
  let charIndex = 0;
  while(row[charIndex * 3 + charIndex] != undefined){
    const box = row.slice(charIndex * 3 + charIndex, charIndex * 3 + charIndex + 3).replace(/\[|\]/g, '')
    if(box.trim()){
...
instructions = instructions.map(i =>
  i.split(/move | from | to /g).filter(c => c).map(Number)
).map(i => {
...
return Object.values(piles).map(p => p.at(-1) || '').join('')